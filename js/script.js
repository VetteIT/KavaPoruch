...
function closeModal() {
  ...
  document.getElementById('chat-now-button').addEventListener('click', function() {
    ...
    // Check if chat is being opened and if the greeting message should be displayed
    if (chatApp.classList.contains('active')) {
...
async function fetchAIResponse(message) {
    const apiKey = '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'; 
    try {
    ...
    if (type === "other") {
        ...
        function typeWriter() {
            if (i < message.text.length) {
                textEl.innerHTML += message.text.charAt(i);
                i++;
